...
controller.player2.onButtonEvent(ControllerButton.A, ControllerButtonEvent.Pressed, function () {
    p2Projectile = sprites.createProjectileFromSprite(assets.image`p2Arrow`, p2, -50, 0)
    pause(1000)
})
sprites.onOverlap(SpriteKind.OtherProjectile, SpriteKind.Enemy, function (sprite, otherSprite) {
	sprite.destroy()
    p2Status.value -= 25
})
...
controller.player1.onButtonEvent(ControllerButton.A, ControllerButtonEvent.Pressed, function () {
    p1Projectile = sprites.createProjectileFromSprite(assets.image`p1Arrow`, p1, 50, 0)
    p1Projectile.setKind(SpriteKind.OtherProjectile)
    pause(1000)
})
